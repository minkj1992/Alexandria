...
class AsyncRecursiveUrlLoader(RecursiveUrlLoader):
    def __init__(
        ...
        self.semaphore = asyncio.Semaphore(semaphore_count)  # Limit the number of concurrent tasks
    ...
    async def aload(self) -> List[Document]:
        ...
        child_documents = await self._async_get_child_links_recursive(self.url, visited) or []
        
        return [doc for doc in ([root_document] + child_documents) if doc is not None]


    async def _async_get_child_links_recursive(self, url: str, visited: Optional[Set[str]] = None, depth: int = 0) -> List[Document]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            sub_documents = await self._fetch_sub_documents(client, absolute_paths, visited, depth)
    ...
    async def _process_links(self, client, links: List[str], visited: Set[str]) -> List[Document]:
        tasks = [self._process_link(client, link) for link in links if link not in visited]
    ...
    async def _process_link(self, client, link: str) -> Optional[Document]:
        async with self.semaphore:
            try:
                ...
                if extracted:
                    return Document(page_content=extracted, metadata=self._gen_metadata(text, link))
            except (httpx.InvalidURL, Exception) as e:
                logger.error(f"Error processing {link}: {e}")
                return None

    async def _fetch_sub_documents(self, client, links: List[str], visited: Set[str], depth: int) -> List[Document]:
        tasks = [self._async_get_child_links_recursive(link, visited, depth + 1) for link in links]
        return [doc for sublist in await asyncio.gather(*tasks) for doc in sublist if doc]
